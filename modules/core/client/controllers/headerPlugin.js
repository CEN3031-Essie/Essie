...
function autoMainMenuHelper() {
	$('.main-menu-wrap>ul>li>a').addClass('main-menu-link').wrapInner('<span></span>').append('<span class="icon-svg icon-caret"><svg><use xlink:href="/media/templates/uf2015/img/spritemap.svg#caret"></use></svg></span>');
...
function displayUFAlert() {
	...
	if (alertContainer) {
		$.getJSON("https://assets.webadmin.ufl.edu/ufalerts/ufalerts.json").done(function(json){
			var timeThreshold = Math.floor((new Date().getTime() / 1000 ) - 10800);
			var lastAlert = json.item[0];
			var alertDate = new Date(parseInt(lastAlert.date));
			var alertTime = alertDate.getTime();
			if (alertTime > timeThreshold) {
				alertContainer.addClass("ufalert").removeClass('big-stat-img').css("background-image","url(/media/templates/uf2015/img/bg-big-stat-alert.jpg)");
				alertContainer.children(".category-tag").html('<span class="icon-svg icon-alert"><svg><use xlink:href="/media/templates/uf2015/img/spritemap.svg#alert"></use></svg></span> '+lastAlert.title);
				alertContainer.children(".big-stat-copy").find("a").attr("href", lastAlert.link).first().text(lastAlert.title);
			}
		});
	}
}
